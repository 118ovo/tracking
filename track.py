...
# Faster profile update for frontal-to-side-view change
PROFILE_UPDATE_ALPHA = 0.06
...
# Normal tracking feature gate
TRACK_UPPER_THRES = 0.48
TRACK_LOWER_THRES = 0.40
TRACK_GLOBAL_THRES = 0.48
TRACK_APPEARANCE_THRES = 0.50

# Re-identification gates
UPPER_REID_THRES = 0.55
LOWER_REID_THRES = 0.50
HEAD_REID_THRES = 0.20
GLOBAL_REID_THRES = 0.54
APPEARANCE_REID_THRES = 0.56
...
def profile_similarity(profile, frame, box):
    ...
    ar_score = min(profile.aspect_ratio, cand.aspect_ratio) / max(
        profile.aspect_ratio, cand.aspect_ratio + 1e-6
    )
    area_score = min(profile.area_ratio, cand.area_ratio) / max(
        profile.area_ratio, cand.area_ratio + 1e-6
    )
...
def passes_tracking_gate(candidate_scores):
    ...
    ok = (
        candidate_scores["upper"] >= TRACK_UPPER_THRES
        and candidate_scores["lower"] >= TRACK_LOWER_THRES
        and candidate_scores["global"] >= TRACK_GLOBAL_THRES
        and app_score >= TRACK_APPEARANCE_THRES
    )
...
def passes_reid_gate(candidate_scores):
    ...
    ok = (
        candidate_scores["upper"] >= UPPER_REID_THRES
        and candidate_scores["lower"] >= LOWER_REID_THRES
        and candidate_scores["head"] >= HEAD_REID_THRES
        and candidate_scores["global"] >= GLOBAL_REID_THRES
        and app_score >= APPEARANCE_REID_THRES
    )
...
def prune_center_trail(center_trail, current_frame_idx, max_trail_frames):
    return [item for item in center_trail if current_frame_idx - item[0] <= max_trail_frames]
...
def main():
    ...
    print("U / L / H / G / A mean Upper-body / Lower-body / Head / Global appearance / Overall.")
    ...
    while True:
        ...
        if len(detections) > 0:
            ...
            if not occluded:
                ...
                passed_candidates = sorted(passed_candidates, key=lambda x: x["app_score"], reverse=True)
